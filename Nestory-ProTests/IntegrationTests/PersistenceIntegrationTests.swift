...
final class PersistenceIntegrationTests: XCTestCase {
    ...
    func testFetchItems_WithPredicate_ReturnsMatchingItems() async throws {
        await MainActor.run {
            ...
            XCTAssertTrue(itemsInCategory?.contains(item1) ?? false)
            XCTAssertTrue(itemsInCategory?.contains(item2) ?? false)
            XCTAssertFalse(itemsInCategory?.contains(item3) ?? true)
        }
    }
