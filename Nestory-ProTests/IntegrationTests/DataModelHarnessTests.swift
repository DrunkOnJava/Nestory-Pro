...
final class DataModelHarnessTests: XCTestCase {
    ...
    func testBulkCreate_500Items_WithRelationships() async throws {
        await MainActor.run {
            ...
            XCTAssertEqual(photos?.count ?? 0, itemCount / 3 + 1, accuracy: 1, "Photos should match expected count")
            XCTAssertEqual(receipts?.count ?? 0, itemCount / 7 + 1, accuracy: 1, "Receipts should match expected count")
